...
bool TileMap::LoadFromTmj(const std::string &filePath, TextureManager &textureManager) {
    ...
    std::sort(m_tileSets.begin(), m_tileSets.end(), [](const TileSet &a, const TileSet &b) {
    ...
    for (const Json &layerJson : document["layers"]) {
        ...
        m_tileLayers.push_back(TileLayer{layer.name, layer.width, layer.height, layer.isVisible, std::move(layer.tiles)});
    }
