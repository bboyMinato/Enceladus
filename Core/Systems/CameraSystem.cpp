...
void CameraSystem::UpdateFollow(CameraComponent& camera, const TransformComponent& target,
                                const SpriteComponent& sprite, RenderSystem& renderSystem, const TileMap& tileMap)
{
	...
	camera.m_viewport.x = std::clamp(camera.m_viewport.x, 0,
	                                 std::max(0, tileMap.GetPixelWidth() - camera.m_viewport.w));
	camera.m_viewport.y = std::clamp(camera.m_viewport.y, 0,
	                                 std::max(0, tileMap.GetPixelHeight() - camera.m_viewport.h));
}
